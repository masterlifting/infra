...
13. **Task kind** — `## Context` contains exactly one `- Task kind: code` or `- Task kind: non-code` marker. Code tasks require C0 and the canonical implementation/build/test/reviewer gates; non-code tasks must not retain those code-only gates.
