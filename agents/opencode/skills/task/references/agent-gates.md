...
Mandatory agent involvement per task phase for code tasks. Gates are **enforced via checkboxes** in the task template — a subtask cannot be ticked complete while its gate checkbox is open.

Language match: pick the team under `agents/software/` that matches the touched code — `dotnet/csharp`, `dotnet/fsharp`, or `rust`. Use `database/sql-engineer`/`database/sql-reviewer` for DB-heavy work and `devops/engineer`/`devops/reviewer` for CI/infra surfaces.
...
5. After C0 fixes change code, re-run the affected reviewer(s) on the new diff before proceeding to C1/C2.
...
- Non-code tasks (pure analysis, docs-only): drop the design gate and C0 at template generation, same as dropping C2–C4 when no review artifact is needed.
- Run independent gate agents **in parallel** — e.g. the C0 board.
- Gate agents follow `rules/software/team.md`: they never run builds or tests themselves and report findings-first with minimal output.
