...
- `C:/Users/andre/.config/opencode/skills/task/references/agent-gates.md` — software-agent ownership, proportional architecture routing, review profiles, and bounded review state machine
...
- Batch TASK.md state updates coherently: recompute progress and validate before each batch is durable (the automatic plugin does this per edit). Durability boundary: validated TASK.md facts are durable; in-session drafts, proposals, and working notes are transient and must not be recorded as durable state.
...
5. **Enforce agent gates** - `references/agent-gates.md` is the canonical software orchestration procedure and owns architecture selection; invoke independent architecture proposals only when that gate requires them. Then run implementation, build, tests, one Discovery review, triage, bounded remediation, and targeted Verification. Parallelize only already-required independent waves (architecture proposals, independent implementation slices, Discovery reviewers, independent remediation items, targeted Verification); keep dependent chains (implementation → build → test → review → remediation) ordered. Preserve required gate checkboxes when refining or replacing subtask 5.
...
4. Revalidate volatile facts only when they are material to the next work: target repo identity; branch/HEAD; working tree state; upstream/base when relevant; PR/MR and CI/check state when relevant; a mutable ticket/spec when the next work depends on it; dependency/API/tool versions only when material. Refresh `## Key Files`, `## References`, `## Decisions`, and `## Open Questions` only when context actually changed. Do not reread demonstrably stable context, and do not keep a large "last loaded" table in the task file.
5. Preserve resume-critical facts in the task file: target branches, touched repos, files modified this session, verification status, open TODOs, and the next approved or pending subtask. Keep the record durable and portable: store only durable conclusions, not transient snapshots.
